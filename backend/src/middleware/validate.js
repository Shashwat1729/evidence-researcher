...
import { isKnownModel, canonicalizeModel } from '../config.js';
...
export function validateResearchBody(body) {
  ...
  // Retired ids canonicalize to Auto (stale saved preferences must not 400).
  const model = body?.model === undefined ? undefined : canonicalizeModel(body.model);
  if (model !== undefined && model !== '' && !isKnownModel(model)) errors.push('model must be one of the offered models');
